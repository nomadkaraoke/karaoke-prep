...
def test_truncate_to_nearest_word(basic_finaliser):
    ...
    assert truncated_exact == exact_title

    title_with_no_space = "a"*60
    truncated_no_space = basic_finaliser.truncate_to_nearest_word(title_with_no_space, max_len)
    assert truncated_no_space == "" # Should truncate to empty string if no space found

# --- Authentication Tests ---

def test_authenticate_youtube_new_token(finaliser_for_yt, mock_google_auth, mock_youtube_service):
...
def test_authenticate_youtube_load_valid_token(finaliser_for_yt, mock_google_auth, mock_youtube_service):
    ...
    mock_google_auth["mock_path_exists"].assert_called_once_with(YOUTUBE_TOKEN_FILE)
    mock_google_auth["mock_pickle_load"].assert_called_once_with(ANY) # Check it tried to load
...
def test_authenticate_youtube_refresh_token(finaliser_for_yt, mock_google_auth, mock_youtube_service):
    ...
    mock_google_auth["mock_flow_instance"].run_local_server.assert_not_called() # Should not run flow
    mock_google_auth["mock_pickle_dump"].assert_called_once() # Should save refreshed token
    mock_google_auth["mock_build"].assert_called_once_with("youtube", "v3", credentials=mock_google_auth["mock_credentials"])
    assert service == mock_youtube_service
...
def test_check_if_video_title_exists_found_reject(mock_input, mock_fuzz_ratio, mock_auth, finaliser_for_yt, mock_youtube_service):
    ...
    assert exists is False # User rejected
...
def test_upload_youtube_success(mock_truncate, mock_check_exists, mock_auth, mock_media_upload, mock_open_file, finaliser_for_yt, mock_youtube_service):
    """Test successful YouTube upload."""
    mock_auth.return_value = mock_youtube_service
    mock_media_upload.return_value = MagicMock() # Mock the media file upload object
    ...
    mock_open_file.assert_called_once_with(YOUTUBE_DESC_FILE, "r")
    ...
    assert insert_call_args[1]['media_body'] == mock_media_upload.return_value
    # Check MediaFileUpload calls with relative paths
    mock_media_upload.assert_has_calls([
    ...
    ], any_order=True) # Order might vary slightly depending on execution
    ...
    assert thumb_call_args[1]['media_body'] == mock_media_upload.return_value # This assumes the mock returns the same instance for both calls, adjust if needed
...
def test_upload_youtube_exists_skip(mock_delete, mock_check_exists, mock_auth, mock_media_upload, finaliser_for_yt, mock_youtube_service):
...
def test_upload_youtube_exists_replace_success(mock_delete, mock_check_exists, mock_auth, mock_media_upload, finaliser_for_yt, mock_youtube_service):
...
def test_upload_youtube_exists_replace_delete_fails(mock_delete, mock_check_exists, mock_auth, mock_media_upload, finaliser_for_yt, mock_youtube_service):
...
def test_upload_youtube_dry_run(mock_check_exists, mock_auth, finaliser_for_yt, mock_youtube_service):
    ...
    mock_check_exists.assert_not_called() # Should skip check in dry run? No, check should still happen.
    mock_youtube_service.videos().insert.assert_not_called()
    mock_youtube_service.thumbnails().set.assert_not_called()
    finaliser_for_yt.logger.info.assert_any_call(pytest.string_containing("DRY RUN: Would upload"))
    assert finaliser_for_yt.youtube_video_id is None
    assert finaliser_for_yt.youtube_url is None
